...
{   
    ...
    if (flags[Flag::E]) {
        ...
        return ObjectCode(Format4(OPTAB[opcode_str].opcode, flags_to_int(flags, 6), address));
    }
    else {
        ...
        return ObjectCode(Format3(OPTAB[inst.opcode].opcode, flags_to_int(flags, 6), disp));
    }
}
...
void MultiPassAssembler::translate(string filename) {
    ...
    // print symbol table
    // cout << "SYMBOL TABLE -> " << SYMTAB.size() << "entries" << std::endl;
